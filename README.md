...
| `deployment.serviceUrlMode` | Controls how the Core container manages FME Flow's service URLs (Data Download, job results, etc.). `ingress` derives service URLs from `deployment.hostname` (default behavior). `internal` forces service URLs to the internal Kubernetes service `http://fmeflowweb:8080` — use when the ingress hostname is not resolvable from inside the cluster; job submission works but Data Download URLs are not browser-reachable. `unmanaged` disables dynamic service URL configuration so URLs set in the FME Flow Web UI persist across pod restarts. **Requires FME Flow 2026.3 or newer** | `ingress` |
...
| `fmeflow.engines.podDeletionCost.enabled` | When `true`, adds a sidecar to every Engine pod that reads a marker file written by the engine container's pod-deletion-cost-watcher and patches the [`controller.kubernetes.io/pod-deletion-cost`](https://kubernetes.io/docs/concepts/workloads/controllers/replicaset/#pod-deletion-cost) annotation on idle ↔ busy transitions. The Kubernetes scheduler then prefers to evict idle engines over busy ones when scaling down. **Requires FME Flow 2026.3 or newer** | `false` |
| `fmeflow.engines.podDeletionCost.busyValue` | Annotation value written when the engine is running a job. Higher = more costly for the scheduler to evict. | `10000` |
| `fmeflow.engines.podDeletionCost.idleValue` | Annotation value written when the engine is idle. | `0` |
| `fmeflow.engines.podDeletionCost.pollIntervalSeconds` | Seconds between job-state checks. Drives both the engine-side watcher (`POD_DELETION_COST_WATCHER_POLL_INTERVAL_SECONDS` env var) and the sidecar's marker-file polling. The annotation is only patched on transitions, not on every poll. | `5` |
| `fmeflow.engines.podDeletionCost.markerFilePath` | Path inside both the engine and sidecar containers where the engine writes and the sidecar reads the marker. The chart mounts an emptyDir at the containing directory and group-owns it via `fsGroup: 1363` so the engine user can write to it. | `/var/run/fmeflow/job-state` |
| `fmeflow.engines.podDeletionCost.bootstrapTimeoutSeconds` | How long the sidecar waits for the marker file to first appear before crash-looping with a clear error. Trips when the engine image is older than FME Flow 2026.3 (and so doesn't ship the watcher script) or something else prevents `POD_DELETION_COST_WATCHER_ENABLED` from being honored. | `60` |
| `fmeflow.engines.podDeletionCost.runAsUser` | UID for the sidecar. The marker file is written world-readable by the engine container so any nonroot UID works. | `65532` |
| `fmeflow.engines.podDeletionCost.image.registry` | Docker registry for the sidecar image. | `docker.io` |
| `fmeflow.engines.podDeletionCost.image.repository` | Repository for the sidecar image. The sidecar talks to the Kubernetes API via `curl`. Any image that provides `/bin/sh` and `curl` works. | `curlimages/curl` |
| `fmeflow.engines.podDeletionCost.image.tag` | Tag for the sidecar image. | `8.20.0` |
| `fmeflow.engines.podDeletionCost.image.pullPolicy` | Image pull policy for the sidecar. | `IfNotPresent` |
| `fmeflow.engines.podDeletionCost.resources` | CPU/memory requests and limits for the sidecar container. | Memory: `32Mi`, CPU: `10m` |
...
| `fmeflow.webserver.showPasswordLoginUI` | Show the user/password fields on the FME Flow login page. Set to `false` to disable standard user/password login and only allow the configured external identity provider. **Requires FME Flow 2026.1.2.1 or newer** | `true` |
...
| `fmeflow.maintenance.idleWait` | Amount of time in seconds an Engine must be idle after processing at least one job before it undergoes maintenance. A value of 0 or less disables maintenance. This feature is available for FME Flow 2026.3+. | `90` |
